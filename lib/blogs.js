...
export function upsertDemoBlogs(blogs = [], now = new Date().toISOString()) {
  ...
  for (const demo of DEMO_BLOGS) {
    ...
    if (idx >= 0) {
      next[idx] = normalizeBlog({
        ...next[idx],
        ...item,
        id: next[idx].id,
        createdAt: next[idx].createdAt || now,
        updatedAt: now,
        // Keep publish status if already published; otherwise use demo status
        status:
          next[idx].status === "published" || demo.status === "published"
            ? "published"
            : "draft",
        publishedAt:
          next[idx].status === "published" || demo.status === "published"
            ? next[idx].publishedAt || now
            : null,
        // Always refresh demo cover assets so launch images stay healthy
