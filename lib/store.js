...
import { normalizeDescription, normalizeTags } from "./tags";
import { normalizeBlog } from "./blogs";

const REDIS_KEY = "bangers:board";
const GIST_FILENAME = "board.json";
const EMPTY = { pending: [], approved: [], rejected: [], blogs: [] };
...
function normalize(store) {
  return {
    pending: (store?.pending || []).map(normalizeItem),
    approved: (store?.approved || []).map(normalizeItem),
    rejected: (store?.rejected || []).map(normalizeItem),
    blogs: (store?.blogs || []).map(normalizeBlog),
  };
}
...
export async function writeStore(store) {
  ...
  const gistConfigured = hasGist();

  const redisOk = await redisSet(next);
  const gistOk = await gistSet(next);
  const fileOk = await fileSet(next);

  // Prevent Redis/Gist split-brain. If Redis write fails, drop the stale key so
  // the next read can fall through to Gist instead of serving old content.
  if (redisConfigured && !redisOk) {
    await redisDel();
    if (!gistOk) {
      throw new Error("Could not save board data to durable storage");
    }
  } else if (gistConfigured && !gistOk && !redisOk) {
    throw new Error("Could not save board data to durable storage");
  }
  ...
  if (!process.env.VERCEL && !redisConfigured && !gistConfigured && !fileOk) {
